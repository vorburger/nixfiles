{ inputs, ... }:
{
  flake.nixosConfigurations.test1-vm = inputs.nixpkgs.lib.nixosSystem {
    system = "x88_64-linux";
    modules = [
      # TODO Modularize all this properly...

      inputs.disko.nixosModules.disko
      ./disko/_boot-and-ext4.nix

      # TODO users/vorburger.nix

      (
        { pkgs, ... }:
        {
          boot.loader.grub.enable = true;
          boot.loader.grub.devices = [ "/dev/vda" ]; # TODO use disko to get device

          # TODO Verify if hostname is automatically set from flake name / attribute name or not?
          networking.hostName = "test1-vm";

          services.openssh.enable = true;
          services.openssh.settings.PermitRootLogin = "prohibit-password";
          virtualisation.forwardPorts = [
            {
              from = "host";
              host.port = 2222;
              guest.port = 22;
              # You can optionally specify the protocol (default is "tcp")
              # protocol = "tcp";
            }
          ];

          users.users.test = {
            isNormalUser = true;
            extraGroups = [ "wheel" ];
            initialPassword = "x";
          };

          users.users.vorburger = {
            openssh.authorizedKeys.keys = [
              "ssh-rsa "
            ];
            isNormalUser = true;
            extraGroups = [
              "wheel"
              # ? "networkmanager"
            ];
            packages = with pkgs; [
              # TODO Replace with https://github.com/vorburger/vorburger-dotfiles-bin-etc/tree/main/dotfiles/home-manager ...
              git
              nano
            ];
          };

          system.stateVersion = "25.05";
        }
      )
    ];
  };
}
