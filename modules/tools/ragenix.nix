{ inputs, ... }:
let
  identitiesData = import ../../identities.nix;
        ...
        program =
          let
            script = pkgs.writeShellScriptBin "write-recipients" ''
              ...
              RECIPIENTS_FILE="$ROOT_DIR/recipients.nix"

              echo "Dynamically deriving recipients from identities.nix..."

              # Evaluate identities.nix dynamically using nix-instantiate
              nix-instantiate --eval --strict --json -E '
                let
                  data = import "'"$ROOT_DIR"'/identities.nix";
                  hosts = data.hostKeys;
