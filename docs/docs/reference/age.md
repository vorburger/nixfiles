...
> For details on how `nixfiles` handles default identity loading via the `ragenix` Fish wrapper while keeping `rage` unaliased, see [Secrets Documentation - User Identities](secrets.md#2-managing-secrets-with-ragenix-cli).
