...
- After making any and for each change to `*.nix` files, always execute `nix develop --command nix-fast-build --flake .#checks.x86_64-linux` (which should be equivalent to `nix flake check`, just faster) to run various tests which verify everything is OK.
